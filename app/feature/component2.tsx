...
const WheelComponent = ({
  ...
  let segments = options?.map((option) => option.option) || [];
  let currentSegment = "";
  ...
  let canva: HTMLCanvasElement | null = document.getElementById(
    canvasId.current
  ...
  const drawSegment = (key: number, lastAngle: number, angle: number) => {
    ...
    ctx.closePath();
    ctx.fillStyle = options[key]?.style?.backgroundColor
      ? options[key]?.style?.backgroundColor
  ...
  const drawWheel = () => {
    ...
    ctx.beginPath();
    ctx.arc(centerX, centerY, 120, 0, PI2, false);
    ctx.closePath();
  ...
  const drawNeedle = () => {
    ...
    ctx.fillStyle = contrastColor;
    ctx.beginPath();
    ctx.moveTo(centerX + 70, centerY - 100);
